...
class PurchaseOrder(models.Model):
    ...
    def button_manager_approve(self, force=False):
        for order in self:
            ...
            if order.company_id.po_approval == True:
                if order.amount_total < self.env.user.company_id.currency_id.compute(order.company_id.manager_validation_amount, order.currency_id):
    ...
    def button_accountant_approve(self, force=False):
        for order in self:
            # Deal with Three Step validation process
            if order.company_id.po_approval == True:
                if order.amount_total < self.env.user.company_id.currency_id.compute(order.company_id.accountant_validation_amount, order.currency_id):
                    order.button_approve()
                else:
